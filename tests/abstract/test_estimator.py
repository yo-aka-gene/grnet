...
def dfs():
    return [
        pd.DataFrame(v)
        for v in [np.ones((5, 5)), np.zeros((10, 2)), np.eye(4), np.random.rand(20, 14)]
    ]
...
def test_init_invalid_dtype_data(not_df):
    for i, v in enumerate(not_df):
        with pytest.raises(AssertionError) as e:
            Estimator(data=v)
        assert f"{v}" in f"{e.value}", f"test failed for {i}-th input: {e.value}"
...
def test_init_invalid_value_n(not_pos_int):
    ...
    for i, v in enumerate(not_pos_int):
        with pytest.raises(AssertionError) as e:
            Estimator(data=df, n=v)
        assert "Invalid" in f"{e.value}", f"test failed for {i}-th input: {e.value}"
...
def test_init_correct_attribute_dtype(dfs):
    for i, v in enumerate(dfs):
        ...
        assert isinstance(
            model.data, pd.core.frame.DataFrame
