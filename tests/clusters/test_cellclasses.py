...
def not_list_of_estimators(estimators):
    return [[*estimators, v] for v in typemolds(Estimator)]


@pytest.fixture
def not_list_of_names():
    return [["cluster1", 123, v] for v in typemolds((str, int))]
...
def not_list_of_colors():
    return [["C0", (0.1, 0.5, 0.3, 0.2), v] for v in typemolds((str, tuple))]


@pytest.fixture
def not_tuple_of_colors():
    return [[(0.1, 0.5, 0.3, 0.2), (0.2, 0.3, 0.4, v)] for v in typemolds(float)]
...
def test_invalid_dtype_models(not_list):
    for i, v in enumerate(not_list):
        ...
        assert "Invalid dtype" in f"{e.value}", f"test failed for {i}-th input {v}"
...
def test_correct_models(estimators):
    ...
    assert clusters.models == expect, "invalid value for self.models"


def test_correct_grns(estimators):
    clusters = CellClasses(models=estimators)
    expect = {i: v.get_matrix() for i, v in enumerate(estimators)}
    assert clusters.grns == expect, "invalid value for self.grns"
...
def test_correct_names(estimators, names):
    ...
    assert clusters.names == expect, "invalid value for self.names"
...
def test_correct_colors(estimators, colors):
    ...
    assert clusters.colors == expect, "invalid value for self.colors"
