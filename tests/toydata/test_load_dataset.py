...
def correct_names():
    ...
    ret = np.unique(
        [
            v.split("/")[-1].split("_")[-1].split(".")[0]
            for v in glob.glob(f"{root}/grnet/toydata/data/*")
        ]
    ).tolist()
    return ret
...
@pytest.fixture
def invalid_names():
    return ["not_data", "invalid", "incorrect"]


def test_available_name_correct_return(correct_names):
    assert (
        available_name() == correct_names
...
def test_load_dataset_correct_return(correct_names):
    for i, v in enumerate(correct_names):
        assert isinstance(
            load_dataset(v), pd.core.frame.DataFrame
...
def test_load_metadata_correct_return(correct_names):
    for i, v in enumerate(correct_names):
        assert isinstance(
            load_metadata(v), pd.core.frame.DataFrame
        ), f"test failedd for {i}-th input {v}"
