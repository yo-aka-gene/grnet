...
def test_invalid_kwargs(not_dict):
    for i, v in enumerate(not_dict):
        ...
        assert "kwargs" in f"{e.value}", f"test failed for {i}-th input {v}"
...
def test_invalid_key(not_str):
    for i, v in enumerate(not_str):
        ...
        assert "key" in f"{e.value}", f"test failed for {i}-th input {v}"
...
def test_invalid_typehint(not_type):
    for i, v in enumerate(not_type):
        ...
        assert "typehint" in f"{e.value}", f"test failed for {i}-th input {v}"
...
def test_not_found():
    kwargs = [{}, {f"a{i}": i for i in range(5)}]
    for i, v in enumerate(kwargs):
        ret = kwarg_mgr(kwargs=v, key="asdf", default=None)
        assert ret is None, f"test failed for {i}-th input: expected None, got {ret}"


def test_found():
    kwargs = [{"asdf": "ghjk"}, {**{f"a{i}": i for i in range(5)}, "asdf": "ghjk"}]
    for i, v in enumerate(kwargs):
        ret = kwarg_mgr(kwargs=v, key="asdf", default=None)
        assert (
            ret == "ghjk"
...
def test_found_and_invalid_type():
    ...
    for i, v in enumerate(kwargs):
        ...
        assert "asdf" in f"{e.value}", f"test failed for {i}-th input"
