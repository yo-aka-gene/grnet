...
def list_of_values():
    return ["a", 0, 1.1, 1 + 4j, np.ones(10)[1], np.pi, True, type, (), [], {}]
...
def _arraylike():
    return [np.ndarray, pd.core.frame.DataFrame, pd.core.indexes.base.Index]
...
@pytest.fixture
def tuple_of_types():
    return [v for v in combinations(_types(), 2)]


@pytest.fixture
def tuple_of_arraylike():
    return [v for v in combinations(_arraylike(), 2)]
...
def test_valid_input_arraylike(arraylike):
    for i, v in enumerate(arraylike):
        ...
        assert np.all(
            [np.all(r == e) for r, e in zip(ret, expect)]
        ), f"invalid return values in {i}-th input\nexpected: {expect}\ngot: {ret}"
