...
@pytest.fixture
def tuple_of_types():
    return [(str, type(v)) for v in _values()]
...
@pytest.fixture
def trues():
    return [True, 1 == 1, (np.ones([1]) == 1)[0]]


@pytest.fixture
def falses():
    return [False, 1 == 0, (np.ones([1]) == 0)[0]]
...
def test_type_invalid_type_declaration(values):
    for i, v in enumerate(values):
        wrong_type = (
            type(_values()[0]) if i == len(_values()) - 1 else type(_values()[i + 1])
        )
        print(v, wrong_type)
        with pytest.raises(AssertionError) as e:
...
def test_type_correct_input_values(values):
    for i, v in enumerate(values):
        assert (
            typechecker(v, type(v), "name") is None
...
def test_type_correct_error_msg(values):
    for i, v in enumerate(values):
        ...
        assert (
            f"{wrong_type}" in f"{e.value}"
        ), f"Failed when the {i}-th input is {v} and the output is {e.values}\n\
                while {wrong_type} should be there"
        assert (
            "asdfghjkl" in f"{e.value}"
        ), f"Failed when the {i}-th input is {v} and the output is {e.values}\n\
