...
@pytest.fixture
def not_df_for_grn():
    const = {"name": "cluster_1", "color": (0.5, 0.5, 0.5, 0.5)}
    return [{**const, "grn": v} for v in typemolds(pd.core.frame.DataFrame)]
...
def not_grn_for_grn(not_grn):
    const = {"name": "cluster_1", "color": (0.5, 0.5, 0.5, 0.5)}
    return [{**const, "grn": v} for v in not_grn]
...
def correct_dict():
    return [
        ...
        {
            "grn": pd.DataFrame(np.eye(5)),
            "name": "a",
            "color": "C0",
            "asdf": np.zeros(12),
        },
        {"grn": pd.DataFrame(np.tri(3)), "name": 0, "color": (0.8, 0.7, 0.6, 0.9)},
...
def test_invalid_dict(invalid_dict):
    for i, v in enumerate(invalid_dict):
        with pytest.raises(AssertionError) as e:
            is_cellclass_dict(v)
        assert f"{v.keys()}" in f"{e.value}", f"test failed for {i}-th input {v}"
...
def test_not_df_for_grn(not_df_for_grn):
    for i, v in enumerate(not_df_for_grn):
        with pytest.raises(AssertionError) as e:
            is_cellclass_dict(v)
        assert f"{v['grn']}" in f"{e.value}", f"test failed for {i}-th input {v}"
...
def test_not_grn_for_grn(not_grn_for_grn):
    for i, v in enumerate(not_grn_for_grn):
        with pytest.raises(AssertionError) as e:
            is_cellclass_dict(v)
        assert "Invalid" in f"{e.value}", f"test failed for {i}-th input {v}"


def test_correct_dict(correct_dict):
    for i, v in enumerate(correct_dict):
        assert is_cellclass_dict(v) is None, f"test failed for {i}-th input {v}"
