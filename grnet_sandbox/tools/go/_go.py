...
def term_array(symbol: str, species: str = "mouse") -> np.ndarray:
    arr = np.concatenate(
        [
            [vv["term"] for vv in v]
            for v in MyGeneInfo()
            .query(symbol, scopes="symbol", fields="go", species=species)["hits"][0][
                "go"
            ]
            .values()
        ]
...
def go_plot(
    ...
    if genes.size <= 1:
        terms = term_array(genes[0])[:top][::-1] if genes.size == 1 else None
        res = (
            pd.DataFrame(
                {
                    "term_name": terms,
        ...
        res = gprofiler(genes).iloc[:top, :].iloc[::-1, :]
    ...
    if genes.size >= 1:
        ...
        ax.set_ylim(-0.5, min(len(res), top) - 0.5)
...
def rename_by_top_sigup(
    ...
    arr = [
        get_deg(data=data, cluster_name=name, pvals_adj=pvals_adj, min_logfc=min_logfc)
        for name in data.uns["rank_genes_groups"]["names"].dtype.names
    ]
    arr = np.array([f"{v[0]}+" if v.size != 0 else "Unknown" for v in arr])
...
def _tally_util(df: pd.DataFrame, groupby: str, x: str, stack: bool = False):
    df = df.value_counts()
    df.name = "counts"
    df = pd.DataFrame(df).reset_index()

    def fmt_df(d: pd.DataFrame) -> pd.DataFrame:
        return (
            d.assign(Proportion=d.counts.cumsum() / d.counts.sum())
            if stack
            else d.assign(Proportion=d.counts / d.counts.sum())
        )

    df = pd.concat(
        [
            fmt_df(df[(df[x] == dn)].sort_values(groupby))
            for dn in np.sort(df[x].unique())
        ],
        axis=0,
    )
    return df
...
def tally(data: ad.AnnData, groupby: str, x: str = "batch"):
    return _tally_util(data.obs[[x, groupby]], x=x, groupby=groupby)


def stacked_tally(data, groupby: str, x: str = "batch"):
    return _tally_util(data.obs[[x, groupby]], x=x, groupby=groupby, stack=True)


def merged_tally(
    data: ad.AnnData, merge_list: list, new_name: str, groupby: str, x: str = "batch"
):
    obs = [x, groupby]
    for i, target in enumerate(merge_list):
        if i == 0:
            ...
            loc |= data.obs[obs] == target
    df = pd.DataFrame(
        np.where(loc, new_name, data.obs[obs]), index=data.obs.index, columns=obs
    )
    return _tally_util(df=df, x=x, groupby=groupby)
