...
class LogisticModel(DORModel):
    def __init__(self, random_state: int = 0, n_trials: int = 100) -> None:
        super().__init__(name="LM", random_state=random_state, n_trials=n_trials)
        return None
    ...
    def plot(
        ...
        **kwargs,
        ...
        ax.scatter(
            1 / (1 + np.e ** (-self.beta * self.x)), self.y, s=s, c=c, label=label
        )
    ...
    def residual_plot(self, ax: plt.Axes = None, label: str = None, **kwargs) -> None:
        if ax is None:
            _, ax = plt.subplots()
        s = kwargs["s"] if "s" in kwargs else 0.1
        c = kwargs["c"] if "c" in kwargs else ".6"
