...
def grnplot(
    ...
    **kwargs
    ...
    if isinstance(data, pd.core.frame.DataFrame):
        ...
        edges = [
            (feat[idx], feat[col])
            for idx, col in zip(*np.where(data == 1))
            if idx != col
        ]
    ...
    for i, v in enumerate(feat):
        ax.annotate(
            v,
            (annot_radius * x[i] + annot_xloc, annot_radius * y[i] + annot_yloc),
            fontsize=fontsize,
        )

    for v in edges:
        ax.plot(
            [x[feat_dict[i]] for i in v], [y[feat_dict[i]] for i in v], color=edgecolor
        )
