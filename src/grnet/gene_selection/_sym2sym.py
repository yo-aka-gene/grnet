...
from grnet.dev import multi_intersec, multi_union
...
def sym_by_intersection(
    markers: List[str], species: str = "human", unique: bool = False
) -> np.ndarray:
    """
...
def sym_by_jaccard(
    ...
    extended_list = multi_union(
        [np.array(markers), sym_by_union(markers, species, unique)]
    ).tolist()
    jim = go_jaccard_matrix(extended_list, species, unique)
