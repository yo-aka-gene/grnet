...
from grnet.dev import multi_intersec, multi_union, typechecker
...
def go_jaccard_matrix(
    ...
    golist = MyGeneInfo().querymany(
        markers, scopes="symbol", fields="go", species=species
    )
    golist = [v for v in golist if "go" in v]
    arr = []
    for idx1, idx2 in product(np.arange(len(markers)), np.arange(len(markers))):
        union = multi_union(
